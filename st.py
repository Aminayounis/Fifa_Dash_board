...
with tab2:
    ...
    if selected_club != None:
        ...
        c3,c4=st.columns(2)
        c3.write(df[df['Club'].isin(sl_clbs)].nlargest(10,'Overall')[['Name','Club','Overall','Skill Moves','Potential']])
        c4.plotly_chart(px.pie((df[df['Club'].isin(sl_clbs)].nlargest(10,'Overall')).groupby('Club')['Club'].count().reset_index(name='count'),names='Club',values='count',hole=.3,title="top players percent per clubs for the top 10"))
        ...
        st.write("Top 10 players among selected clubs")
        c5,c6=st.columns(2)
        c5.write(df[df['Club'].isin(selected_clubs)].nlargest(10,'Overall')[['Name','Club','Overall','Skill Moves','Potential']])
        c6.plotly_chart(px.pie((df[df['Club'].isin(selected_clubs)].nlargest(10,'Overall')).groupby('Club')['Club'].count().reset_index(name='count'),names='Club',values='count',hole=.3,title="top players percent per clubs for the top 10"))
